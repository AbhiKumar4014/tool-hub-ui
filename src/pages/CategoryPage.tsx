...
export function CategoryPage() {
  ...
  const filteredTools = tools.filter(
    tool => tool.category.toLowerCase() === formattedCategory.toLowerCase()
  );
    ...
    <div className="pt-24 pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">{formattedCategory} Tools</h1>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Explore the top {formattedCategory.toLowerCase()} tools available.
          </p>
        </div>
        {filteredTools.length ? (
          ...
          <p className="text-center text-gray-600">No tools available for this category.</p>
        )}
      </div>
    </div>
  );
